...
def check_environment_causality(object_a: ObjectCandidate, object_b: ObjectCandidate) -> bool:
    """
    ENVIRONMENT CAUSALITY LAW (SELECTION GATE)
    
    This law influences pair selection by rejecting pairs where Object B cannot
    plausibly emerge, form, or naturally result from Object A's environment.
    It defers generation but never blocks it.
    
    CORE RULE:
    A valid hybrid must be causally explained by a shared environment.
    Object B must appear to emerge, form, or naturally result from the environment of Object A.
    The environment is NOT a background - it is the generating force of the hybrid.
    
    If Object B could simply be placed on Object A without changing its nature,
    the hybrid is INVALID.
    
    Silhouette similarity alone is insufficient - environmental causality is required.
    
    Args:
        object_a: ObjectCandidate for object A (the environment source)
        object_b: ObjectCandidate for object B (must emerge from A's environment)
    
    Returns:
        True if Object B can plausibly emerge from Object A's environment, False otherwise.
        Returns False on any error (API failure, etc.) to allow fallback behavior.
    ...
    # Derive environment for Object A
    environment_a = derive_environment_for_object(object_a.name)
    
    # Build prompt for environment causality evaluation
    prompt = f"""You are evaluating ENVIRONMENT CAUSALITY between two objects.

OBJECT A: {object_a.name}
OBJECT A's ENVIRONMENT: {environment_a}
OBJECT B: {object_b.name}

ENVIRONMENT CAUSALITY LAW (MANDATORY):
...
It is the generating force of the hybrid.

CRITICAL TEST:
Can Object B plausibly emerge, form, or naturally result from Object A's environment ({environment_a})?

If Object B could simply be placed on Object A without changing its nature,
the hybrid is INVALID.

Silhouette similarity alone is INSUFFICIENT.
Environmental causality is REQUIRED.

INVALID EXAMPLES (reject these):
- Object B is just placed on Object A's surface (no causal relationship)
- Object B exists independently and could exist anywhere (no environmental generation)
- Object B is merely associated with Object A conceptually (no physical causality)
- Object B is a tool used in Object A's environment but doesn't emerge from it
- Object B is a decoration or accessory placed on Object A

VALID EXAMPLES (accept these):
- Object B forms naturally from processes in Object A's environment
- Object B emerges as a result of Object A's environmental conditions
- Object B is generated by forces, materials, or processes inherent to Object A's environment
- Object B appears to be created, shaped, or formed by Object A's environmental context
- Object B is a natural outcome or product of Object A's environmental domain

EVALUATION CRITERIA:
1. Can Object B plausibly emerge from {environment_a}?
2. Is there a causal relationship where Object A's environment generates or forms Object B?
3. Would Object B's nature change if it emerged from Object A's environment vs. being placed there?
4. Is the relationship more than just "Object B exists near Object A"?

If no plausible environmental causality exists, the pair is INVALID.
...
Respond with ONLY "VALID" or "INVALID" followed by a brief one-sentence explanation."""
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",  # Use cost-effective model for this check
            messages=[
                {"role": "system", "content": "You are a strict evaluator of environment causality. You must be precise and reject pairs where Object B cannot plausibly emerge from Object A's environment."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,  # Deterministic evaluation
            max_tokens=100
        ...
        # Check if result indicates VALID causality
        is_valid = result_text.startswith("VALID")
        
        if is_valid:
            logger.info(f"ENV_CAUSALITY_CHECK PASSED A={object_a.name} env={environment_a} B={object_b.name}")
            return True
        else:
            logger.warning(f"ENV_CAUSALITY_CHECK FAILED A={object_a.name} env={environment_a} B={object_b.name} reason={result_text[:100]}")
            return False
            
    except Exception as e:
        # If LLM check fails, log error but be conservative: reject the pair
        logger.error(f"ENV_CAUSALITY_CHECK_ERROR A={object_a.name} B={object_b.name} error={str(e)}")
        # Conservative approach: reject if check fails
        return False
